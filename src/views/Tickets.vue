...
const tickets = computed(() => routeData.value.tickets?.items ?? []);
const filteredTickets = computed(() =>
  tickets.value.filter((ticket) => {
    ...
    if (filters.value.department && !ticket.department.toLowerCase().includes(filters.value.department.toLowerCase())) return false;
    if (filters.value.dateFrom && ticket.date < filters.value.dateFrom) return false;
    if (filters.value.dateTo && ticket.date > filters.value.dateTo) return false;
    return true;
  }),
);

function resetFilters() {
  filters.value = { status: "", priority: "", department: "", dateFrom: "", dateTo: "" };
}
</script>
  ...
  <div class="p-4 md:p-6">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex items-center gap-2">
        <button class="rounded-lg border border-gray-300 px-4 py-2 text-sm" @click="showFilters = !showFilters">
          <i class="ri-filter-line mr-2"></i>Фильтры
        </button>
        <button class="rounded-lg bg-teal-600 px-4 py-2 text-sm text-white" @click="openCreateTicketModal">
    ...
    <section v-if="showFilters" class="mb-6 rounded-xl bg-white p-4 shadow-sm">
      <div class="mb-4 flex items-center justify-between">
        ...
        <button class="text-sm text-teal-600" @click="resetFilters">Сбросить все</button>
      </div>
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <select v-model="filters.priority" class="rounded-lg border border-gray-300 px-3 py-2 text-sm">
    ...
    <div v-if="filteredTickets.length" class="space-y-4">
      <article v-for="ticket in filteredTickets" :key="ticket.id" class="rounded-xl bg-white p-4 shadow-sm md:p-6">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="mb-2 flex flex-wrap items-center gap-2">
              <h3 class="text-base font-semibold text-gray-900 md:text-lg">{{ ticket.title }}</h3>
              <span :class="`rounded-full px-2 py-1 text-xs font-medium ${ticket.status === 'open' ? 'bg-blue-100 text-blue-800' : ticket.status === 'in_progress' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`">
                {{ ticket.statusText }}
              </span>
              <span :class="`rounded-full px-2 py-1 text-xs font-medium ${ticket.priority === 'high' ? 'bg-orange-100 text-orange-800' : ticket.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`">
                {{ ticket.priorityText }}
              </span>
            </div>
          ...
          <div class="flex items-center gap-1">
            <button class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600" @click="openEditTicketModal(ticket.id)">
              <i class="ri-edit-line"></i>
            </button>
            <button class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600" @click="openDeleteTicketModal(ticket.id)">
    ...
    <div v-else class="rounded-xl bg-white p-8 text-center text-sm text-gray-500 shadow-sm">
