...
const menuItems = [
  ...
  { path: "/settings", icon: "ri-settings-3-line", label: "Настройки" },
];
</script>

<template>
  <aside :class="`flex flex-col bg-gray-900 text-white transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`">
    <div class="flex items-center justify-between border-b border-gray-800 p-4">
      ...
      <button class="rounded-lg p-2 hover:bg-gray-800" @click="isCollapsed = !isCollapsed">
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto p-4">
      <RouterLink
        ...
        :key="item.path"
        :to="item.path"
        :class="`flex items-center gap-3 rounded-lg px-3 py-2 transition-colors ${route.path === item.path ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-800'}`"
      >
    ...
    <div class="border-t border-gray-800 p-4">
      <RouterLink to="/profile" :class="`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`">
